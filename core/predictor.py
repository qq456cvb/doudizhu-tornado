...
from core.extra.utils import get_mask, get_minor_cards, train_fake_action_60, get_masks, test_fake_action
from core.extra.utils import get_seq_length, pick_minor_targets, to_char, to_value, get_mask_alter, get_mask_onehot60
...
class Predictor:
    ...
    def get_combinations(self, curr_cards_char, last_cards_char):
        if len(curr_cards_char) > 10:
            ...
            assert np.all(['0' not in a for a in augment_action_space]) and np.all(
                ['w' not in a for a in augment_action_space]) and np.all(
                ['W' not in a for a in augment_action_space])
            if len(last_cards_char) > 0:
